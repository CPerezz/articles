...
Why does 8151 block `ecrecover` at all, instead of letting each account choose? Because the contracts it affects are exactly the ones that can't ask your account anything. An [ERC-2612](https://ercs.ethereum.org/ERCS/erc-2612) `permit` takes a bare `v, r, s`, runs `ecrecover`, compares addresses, and it's immutable. If that kept working after you moved to code, your old key alone could still sign away your tokens there, around your wallet's cosigners and limits, and forever if that key ever leaks or gets broken by a quantum computer. A fresh contract that never had a key has the same hole today: whoever grinds out a colliding key (the `2^80` attack 3607 exists for) could sign permits for it. Contracts that check for code and ask your account through ERC 1271 keep working either way, and there your wallet decides, with its own rules.

So the flexibility is still there, it just comes with the shape. Want plain `ecrecover` apps to keep trusting your key? Stay a delegate with the key on, and 8151 leaves you alone. Go code, and your code speaks for you. A per account switch on top would need a new flag the precompile reads from state, and every account that flips it is back to "the key walks around the rules".
