...
ARTICLES = [
    ...
    {
        # Prebuilt entry: the deliverable is generated by
        # state-db-perf-divergence/gen_state_db_report.py, not converted from
        # markdown, so build_article() skips it and the card links straight at
        # the report file.
        "folder": "state-db-perf-divergence",
        "prebuilt": True,
        ...
        "blurb": "Identical EEST bloatnet runs report wildly different MGas/s on three geth "
                 "databases. The cause turned out to be a 380 MiB pathdb journal shipped inside "
...
def build_landing():
    cards = "\n".join(_fill(CARD, {
        "HREF": a.get("href", a["folder"] + "/"), "TITLE": html.escape(a["card_title"]),
