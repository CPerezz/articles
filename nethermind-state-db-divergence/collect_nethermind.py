...
def besu_reference():
    ...
    These were frozen literals computed over a 108-cell set that pooled the overhead_baseline
    control rows in with the measurement rows. The controls touch no account state and sit at
    parity while reading almost no bytes, so pooling them pulled Besu's ratios toward 1.0 and
    understated the byte gap (0.962x published against 0.908x measured; 2.92x the bytes against
    10.06x). Deriving them here means a re-cut of the Besu study moves this table instead of
    silently contradicting it.

    Method matches this study's own ratios: per-cell median, cells being opcode x account_mode,
